//! I2C async driver example (MPU6500 sensor)
...
bind_interrupts!(struct Irqs {
    FLEXCOMM4 => InterruptHandler<peripherals::I2C4>;
});
...
async fn main(_spawner: Spawner) {
    ...
    let mut i2c = I2c::new(p.I2C4, scl, sda, Irqs, Config::default());

    loop {
        // write_read example
        info!("Reading MPU6500 chip ID...");
        let mut buf = [0u8; 1];
        let res = i2c.write_read(MPU6500_ADDR, &[WHO_AM_I], &mut buf).await;
        match res {
            Ok(_) => {
